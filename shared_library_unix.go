...
static const char* priority_auto_router_shared_object_path() {
	Dl_info info;
	if (dladdr((void*)&PriorityAutoRouterPluginCall, &info) == 0 || info.dli_fname == NULL) {
		return NULL;
	}
...
func sharedLibraryPath() string {
	sharedObjectPath := C.priority_auto_router_shared_object_path()
